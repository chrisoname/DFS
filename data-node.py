...
class DataNodeTCPHandler(SocketServer.BaseRequestHandler):
	...
	def handle_put(self, p):
		...
		try:
			...
			chunkSize = int(self.request.recv(1024))
			print "chunkSize received: ", chunkSize, "\nSending SEND_TO_ME"
			self.request.sendall("SEND_TO_ME")
			...
			while 1:
				if len(chunk) == chunkSize:
					break
				print "Current length of chunk: ", len(chunk)
				print "Final chunkSize:         ", chunkSize
			...
			print "Error datanode handle_get %s" % e
	...
	def handle_get(self, p):
		...
		l = rfile.read()
		print "Sending get response: filesize then file..."
		self.request.sendall(str(len(l)))
		response = ''
		print "Waiting for OK"
		while response != "OK":
			response = self.request.recv(1024)
		print "Sending chunk... len = ", len(l)
